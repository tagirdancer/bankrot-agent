...
"""
import httpx, json, re, os, asyncio, io
...
Если дисконт > 50% и нет документов — risk_level = высокий"""
    ...
    return {
        ...
        "what_to_check":    "1. Выписка ЕГРН 2. Долги ЖКХ 3. Прописанные лица 4. Состояние объекта",
        "action":           action,
        "verdict":          action,
    }
...
async def analyze_lot(lot: dict) -> dict:
    ...
    return {
        ...
        _what = expert.get("what_to_check","") or ""
        _is_template = (
            not _what or
            (_what.count("ЕГРН") > 0 and _what.count("ЖКХ") > 0 and
             _what.count("Прописан") > 0 and lot_type not in ("авто","земля"))
        )
        if _is_template:
            _checks = {
                "квартира":    "1. Выписка ЕГРН — история переходов права\n2. Долги ЖКХ через управляющую компанию\n3. Прописанные лица — потребуют выписки\n4. Осмотр квартиры до подачи заявки\n5. Кадастровая стоимость vs цена торгов",
                "апартаменты": "1. Статус нежилого — прописаться нельзя\n2. Долги по коммунальным платежам\n3. Управляющая компания — тарифы\n4. Назначение здания и ВРИ\n5. Возможность сдачи в аренду",
                "дом":         "1. Выписка ЕГРН дома И участка отдельно\n2. Категория земли — ИЖС или СНТ\n3. Прописанные лица\n4. Коммуникации — газ электричество вода\n5. Состояние фундамента и кровли",
                "коммерция":   "1. Выписка ЕГРН — аресты и обременения\n2. Действующие арендаторы и договоры\n3. Долги по коммунальным платежам\n4. Разрешённое использование помещения\n5. Пожарная безопасность",
                "земля":       "1. Категория и ВРИ — что можно строить\n2. Выписка ЕГРН — охранные зоны\n3. Коммуникации рядом с участком\n4. Межевание сделано или нет\n5. Споры по границам с соседями",
                "авто":        f"1. VIN {vin if vin else 'номер'} на гибдд.рф — ограничения\n2. История ДТП через avtocod.ru\n3. Пробег и техническое состояние\n4. Осмотр до торгов обязателен\n5. Стоимость восстановления если битый",
                "гараж":       "1. Выписка ЕГРН — право собственности\n2. Долги по взносам ГСК\n3. Наличие электричества\n4. Состояние ворот и перекрытий\n5. Подходит ли по размеру",
                "бизнес":      "1. Финансовая отчётность за 3 года\n2. Реальные активы vs балансовая стоимость\n3. Долги кредиторам\n4. Действующие контракты\n5. Причина банкротства",
                "прочее":      "1. Выписка ЕГРН или паспорт объекта\n2. Обременения и аресты\n3. Рыночная стоимость у независимого оценщика\n4. Осмотр объекта\n5. Юридическая история",
            }
            _what = _checks.get(lot_type, "1. Выписка ЕГРН\n2. Обременения и долги\n3. Осмотр объекта")
        "what_to_check":  _what,
        "action":         action,
